...
```bash
aws-toolkit-experiment$ sam build
```
...
```bash
aws-toolkit-experiment$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
aws-toolkit-experiment$ sam local start-api
aws-toolkit-experiment$ curl http://localhost:3000/
```
...
```bash
aws-toolkit-experiment$ sam logs -n HelloWorldFunction --stack-name aws-toolkit-experiment --tail
```
...
```bash
aws-toolkit-experiment$ cd hello-world
...
```bash
sam delete --stack-name aws-toolkit-experiment
```
